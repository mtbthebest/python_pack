...
#             padding_idx=cfg.padding_idx, reduction='mean')
#         # TODO
#         self.fp_16opt = FP16Optimizer(
#             self.model, grad_clip=cfg.grad_clip, loss_scale=1)
#         self.optimizer = LAMB(self.fp_16opt.fp32_params,
#                               lr=cfg.train.lr,
#                               weight_decay=1e-5)
...
#                 # assert torch.any(torch.isinf(out)).item() is False
#                 # float_loss = self.loss_fn(out.float(), dec_tgt)
#                 # loss = float_loss.half()
#                 float_loss = None
...
#                 steps += 1
#                 update_flag = steps and (steps % update_steps == 0)

#                 is_successful = self.fp_16opt.step(loss,
...
#                     if steps % (update_steps * 25000) == 0 or not is_successful: 
...
#             self.train_loader.sampler.set_epoch(5)
#             for j, (src, tgt, dec_tgt) in enumerate(self.train_loader):
#                 src, src_length = src
#                 tgt, tgt_length = tgt
#                 dec_tgt, dec_tgt_length = dec_tgt
...
#                 samples = dict()
#                 samples['tensors'] = {
#                     'src': src.clone(), 'tgt': tgt.clone(), 'dec_tgt': dec_tgt.clone()}
#                 samples['text'] = {'src': [], 'tgt': [], 'dec_tgt': []}
#                 for k in range(sample_num):
#                     for key, tensor in zip(['src', 'tgt', 'dec_tgt'], [src, tgt, dec_tgt]):
#                         samples['text'][key].append(
#                             [self.tokenizer.detokenize(tensor[k].numpy())])
...
class Trainer:
    ...
    def __init__(self, cfg: Config, train_loader, tokenizer, generator, seeds):
        ...
        self.fp_16opt = FP16Optimizer(
            self.model, grad_clip=cfg.grad_clip, loss_scale=1, max_scale=16, dls_upscale_interval=128*2)

        self.optimizer = torch.optim.Adam(
            self.fp_16opt.fp32_params, 
            lr=cfg.train.lr, )
            ...
            for j, (src, tgt, dec_tgt) in enumerate(self.train_loader):
                ...
                if len(batch_data['src']) == update_steps:
                    ...
                    for k in range(update_steps):
                        ...
                        self.fp_16opt.step(loss,
                                            float_loss=None,
                                            optimizer=self.optimizer,
                                            scheduler=self.scheduler,
                                            update=k==(update_steps - 1))
