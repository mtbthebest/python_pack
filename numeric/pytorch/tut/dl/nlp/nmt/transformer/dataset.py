...
from sampler import RandomSampler, BucketSampler
...
def build_collate_fn(batch_first=False, parallel=True, sort=False):
    ...
    def collate_seq(seq):
        ...
        # lengths = torch.tensor([len(s) for s in seq]).to(torch.int64)
        # batch_length = config.seq_length

        shape = (len(seq), batch_length)

        seq_tensor = torch.full(shape, config.padding_idx, dtype=torch.int64)
    ...
    def parallel_collate(seqs):
        ...
        src_tensor, src_len = collate_seq(src_seqs)
        tgt_tensor, tgt_len = collate_seq(tgt_seqs)
        dec_tgt_tensor = tgt_tensor.new(tgt_tensor.size(0), tgt_tensor.size(1))
        dec_tgt_tensor[:, :-1].copy_(tgt_tensor[:, 1:])
        dec_tgt_tensor[:, -1] = 0
        dec_tgt_len = tgt_len - 1
        
        return [(src_tensor, src_len,), (tgt_tensor, tgt_len), (dec_tgt_tensor, dec_tgt_len)]
