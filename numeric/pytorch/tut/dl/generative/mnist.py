    ...
    def load_mnist(path, which='train'):
        ...
        if which == 'train':
            labels_path = os.path.join(
                path, 'emnist-digits-train-labels-idx1-ubyte')
            images_path = os.path.join(
                path, 'emnist-digits-train-images-idx3-ubyte')
        elif which == 'test':
            labels_path = os.path.join(
                path, 'emnist-digits-test-labels-idx1-ubyte')
            images_path = os.path.join(
                path, 'emnist-digits-test-images-idx3-ubyte')
        else:
        ...
        images = images.reshape(-1,
                                int(np.sqrt(images.shape[1])), int(np.sqrt(images.shape[1])))
