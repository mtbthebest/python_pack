    ...
    def load_mnist(path, which='train'):
        ...
        if which == 'train':
            ...
            images_path = os.path.join(
                path, 'emnist-letters-train-images-idx3-ubyte')
        elif which == 'test':
            labels_path = os.path.join(
                path, 'emnist-letters-test-labels-idx1-ubyte')
            images_path = os.path.join(
                path, 'emnist-letters-test-images-idx3-ubyte')
        else:
...
class EMNISTLettersTrain(EMNISTLetters):
...
class EMNISTLettersTest(EMNISTLetters):
